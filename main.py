...
def preprocessing_df(df, column=0, MA_condition=0, window = 0):
    df = df.T
    df = df.iloc[1:, column]
    index_list = list()
    item_list = list()
...
def seperate_train_test(df, target, base):
    ...
    y_train = df.loc[min_index_next:train_end_y, target]
    y_test = df.loc[train_end_y:, target]

    return X_train, X_test, y_train, y_test

# def seperate_train_val(df, target, base):
#     min_index_next = sorting_index_min(df_list).index[predict_gap]
#     train_end = df.index.values[base]
#     train_end_y = df.index.values[base+predict_gap]
#     val_end = df.index.values[base+30]
#     val_end_y = df.index.values[base+30+predict_gap]
#
#     X_train = df.loc[:train_end, df.columns != target]
#     X_val = df.loc[train_end:val_end, df.columns != target]
#     y_train = df.loc[min_index_next:train_end_y, target]
#     y_val = df.loc[train_end_y:val_end_y, target]
...
if __name__ == '__main__':
    ...
    df = pd.concat([df_list[0], df_list[1], df_list[2], df_list[3], df_list[4], df_list[5], df_list[6], df_list[7], df_list[8], df_list[9], df_list[10], df_list[11]], axis=1)
    df.columns = ['deisel', 'interest', 'm_currency', 'current_account', 'PPI', 'spot_rate(dollor)', 'CI', 'house', 'house_price', 'kospi', 'deposit_rate', 'CPI']
    print(df)
    ...
    X_train, X_test, y_train, y_test = seperate_train_test(df, target, 156)
    #166번째 인덱스를 기준으로 train set과 test set을 분리한다.
    X_train, X_test = drop_column(X_train, X_test, [])
    #예측에 불필요한 변수들을 리스트에 담아 drop한다.

    model_list = [LinearRegression(), Lasso(), ann.MLPRegressor(hidden_layer_sizes=(50,50,50))]
    model_dict = {'Linear' : model_list[0], 'Lasso':model_list[1], 'MLPRegressor':model_list[2]}
