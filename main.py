...
def load_data():
    ...
    GDP = preprocessing_df(GDP, 1) #실질경제성장률
    df_list.append(GDP)
...
if __name__ == '__main__':
    ...
    df_list = load_data()
    df = pd.concat(
        [df_list[0], df_list[1], df_list[2], df_list[3], df_list[4], df_list[5], df_list[6], df_list[7], df_list[8],
         df_list[9], df_list[10], df_list[11], df_list[12], df_list[13], df_list[14], df_list[15]], axis=1)
    df.columns = ['gasoline', 'loan_rate', 'currency_vol', 'current_account', 'PPI', 'Won/dollar_rate', 'CLI',
                  'real_estate(volume)', 'real_estate(price)', 'kospi200', 'deposit_rate', 'gold', 'population', 'dept',
                  'GDP','CPI']
    print(df)
    ...
    for predict_gap in range(1, predict_range + 1):
        ...
        # 시험용
        X_train, X_test, y_train, y_test = seperate_train_test(df, target, int(len(df) * 0.985), predict_gap)
        # 예측용
        ...
        # X_train, X_test = drop_column(X_train, X_test, ['currency_vol', 'current_account', 'PPI', 'CLI',
        #           'real_estate(volume)', 'real_estate(price)', 'population', 'dept',
        #           'GDP'])
        # 금 변수 선택

        X_train, X_test = drop_column(X_train, X_test, ['real_estate(volume)', 'gold', 'population', 'dept',
         'GDP'])
        # 코스피200 변수 선택

        # X_train, X_test = drop_column(X_train, X_test,['gasoline', 'current_account', 'PPI', 'Won/dollar_rate', 'CLI',
        #                                                'gold', 'dept'])
        # 부동산매매가격 변수 선택

        #X_train, X_test = drop_column(X_train, X_test,['gasoline','current_account', 'PPI', 'CLI',
                  # 'real_estate(volume)', 'real_estate(price)', 'gold', 'population', 'dept',
                  # 'GDP'])
        # 원/달러환율 변수 선택
    ...
    Ridge_coef = np.array(model_list[0].coef_)
    # MLP_coef = np.array(model_list[1].coefs_)
    # RF_coef = np.array(model_list[2].)
    coef_list = list([Ridge_coef])

    now_num = -1
    # 예측하고자 하는 시간(인덱스) ex- 0이면 test 데이터에서 첫번째인 2017년 11월 데이터)
    lastest_data = np.array(X_final.iloc[now_num, :])
    # 가장 최신인(2022-11) 예측을 위한 독립변수들. predict_gap 이후 예측을 위해
    print(X_final.iloc[now_num, :])
    now = y_final[now_num]

    Ridge_pred = model_list[0].predict(lastest_data.reshape(1, -1))[-1]
    MLP_pred = model_list[1].predict(lastest_data.reshape(1, -1))[-1]
    RF_pred = model_list[2].predict(lastest_data.reshape(1, -1))[-1]
    Ridge_return = (Ridge_pred - now) / now
    MLP_return = (MLP_pred - now) / now
    RF_return = (RF_pred - now) / now
    print('현재 : ', now)
    print('\n릿지 예측 : ', Ridge_pred)
    print('인공신경망 예측 : ', MLP_pred)
    print('랜덤포레스트 예측 : ', RF_pred)

    asset_dict = {'gold': '금', 'real_estate(price)': '서울 부동산', 'kospi200': '국내주식', 'Won/dollar_rate': '달러'}

    print(f'\n지금 {asset_dict[target]} 매입하면 {predict_range}개월 뒤')

    print(f'(릿지회귀) 예측 수익률 : ', round(Ridge_return * 100), '%')
    print(f'(인공신경망) 예측 수익률 : ', round(MLP_return * 100), '%')
    print(f'(랜덤포레스트) 예측 수익률 : ', round(RF_return * 100), '%')

    returns = raw_df[['gold', 'real_estate(price)', 'kospi200', 'Won/dollar_rate']].pct_change()
    vol_gold = returns['gold'].std() * math.sqrt(250)
    vol_real_estate = returns['real_estate(price)'].std() * math.sqrt(250)
    vol_kospi = returns['kospi200'].std() * math.sqrt(250)
    vol_dollar = returns['Won/dollar_rate'].std() * math.sqrt(250)
    covs = returns.cov()
    print(vol_dollar)